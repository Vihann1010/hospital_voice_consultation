...
<<<<<<< HEAD
    ...
    async def finalise_billing(
        ...
        *,
        ...
        """Turn the accrued charges into a real, numbered invoice.

        Until this runs, an admission has a running total but no bill: the
        charges exist as rows against the stay, outside the invoice sequence
        that the finance screens and the auditor actually read.

        Charges are grouped by category rather than posted line by line. A
        three-week stay accrues twenty-one bed rows and as many nursing rows,
        and a patient handed a fifty-line bill for one item repeated cannot
        check it. The detail stays on the admission for anyone who asks.

        The advance taken at admission is recorded as a payment against this
        invoice, so the balance shown is what the family actually owes at the
        counter.
        """
        ...
        if admission.final_invoice_id is not None:
            raise IPDError(
                "This admission has already been billed. Cancel that invoice "
        ...
        rows = await self.session.execute(
            ...
            .where(AdmissionCharge.admission_id == admission_id)
            .group_by(AdmissionCharge.category)
            .order_by(AdmissionCharge.category)
        )
        grouped = rows.all()
        if not grouped:
            raise IPDError(
                "There is nothing to bill on this admission. Run the daily "
                "accrual first."
            )

        label = {
            ChargeCategory.BED: "Bed charges",
            ChargeCategory.NURSING: "Nursing charges",
            ChargeCategory.DOCTOR_VISIT: "Doctor visits",
            ChargeCategory.PROCEDURE: "Procedures",
            ChargeCategory.INVESTIGATION: "Investigations",
            ChargeCategory.MEDICINE: "Medicines",
            ChargeCategory.CONSUMABLE: "Consumables",
            ChargeCategory.OXYGEN: "Oxygen",
            ChargeCategory.OTHER: "Other charges",
        }
        items = [
            {
                "description": f"{label.get(category, category.value)} "
                               f"({count} {'day' if category is ChargeCategory.BED and count == 1 else 'days' if category is ChargeCategory.BED else 'item' if count == 1 else 'items'})",
                "unit_rate_paise": int(total),
                "quantity": 1,
                "tax_percent": 0,
            }
            for category, count, total in grouped
        ]

        reception = ReceptionService(self.session)
        try:
            invoice = await reception.create_invoice(
                visit_id=admission.visit_id,
                patient_id=admission.patient_id,
                items=items,
                invoice_discount_paise=discount_paise,
                discount_reason=discount_reason,
                discount_approved_by=created_by_name if discount_paise else None,
                payer_type=payer_type or PayerType.SELF_PAY,
        ...
        admission.final_invoice_id = invoice.id

        # Mark the charges billed so a second run cannot sweep them again.
        billed = await self.session.execute(
            select(AdmissionCharge).where(
                AdmissionCharge.admission_id == admission_id,
                AdmissionCharge.is_billed.is_(False),
            )
        )
        for charge in billed.scalars().all():
            charge.is_billed = True

        # The deposit is money already taken; it belongs on this invoice as a
        # payment, or the family is asked for the full amount a second time.
        if admission.advance_paid_paise > 0:
            settle = min(admission.advance_paid_paise, invoice.total_paise)
            # Applied directly rather than through record_payment: the money
            # was taken at admission, often days earlier and at a different
            # counter, so routing it through today's cash session would make
            # tonight's drawer look over by the whole deposit.
            invoice.paid_paise += settle
            invoice.status = (
                InvoiceStatus.PAID
        ...
        logger.info(
            "ipd_invoice_raised",
            extra={"ip_number": admission.ip_number,
